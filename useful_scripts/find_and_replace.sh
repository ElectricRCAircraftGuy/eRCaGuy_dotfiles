...
print_help() {
    ...
    echo ''
    echo 'Usage:'
    echo '`find_and_replace <path> <file_regex> <string_regex> <replacement_str> [-w]`'
    ...
    echo '  file_regex      = regular expression to filter which filenames you'"'"'d like to search in;'
    echo '                    use "" or ".*" to match any filename'
    echo '  string_regex    = regular expression for the string to replace in each file'
    echo '  replacement_str = string to overwrite on top of the matching string' 
